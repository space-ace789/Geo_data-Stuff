...
# Extracts the bioclimatic data for the presence and background points
...
#| echo: false
# Prints results of the evaluation function
...
#| echo: false
#| fig-cap: "Figure 4: Map showing predicted probabilities of species 1 occurrence based on predictions from model 4."

plot(species1_pred, main = "GLM Probability of Occurrence")
...
```{r Plotting species 2 GLM Occurence Map}
#| echo: false
#| fig-cap: "Figure 9: Map showing predicted probabilites of species 2 occurrence based on predictions from model 1."
#| fig-align: 'center'
plot(species2_pred, main = "GLM Probability of Occurrence")
...
```{r Species 2 Current Distribution Map}
#| echo: false
...
```{r Plotting predicted probability of both species occurring between the current distributions }
#| echo: false
#| fig-cap: "Figure 11: Map showing the probability that between 0 and 2 species occur in a certain area, yellow represents high likelihood of both species occurring and therefore areas of overlap"
plot(combined, main = "Overlap of GLM Occurrences")
...
#| echo: false
print(paste("Degree of Overlap =", overlap_metric))
```
...
#| code-fold: true
#| code-summary: "Calculating presence and absence of species 2 and species 1 coordinates"
...
# Adds a column for species 2 pa data to the species1_env_pa_data dataframe.
...
#| echo: false
#| fig-cap: "Figure 13: Table comparing the AIC's of several models for the effect of species 2 and climate on species 1"
...
```{r Null model against species effect}
#| echo: false
# Compares the fit of models
anova(null_model, no_climate_model, test = "Chisq")
```

###### Species vs Species and Climate 1

```{r Just species model against climate model 1}
#| echo: false
anova(no_climate_model, no_interaction_model2, test = "Chisq")
```

###### Species-Climate Interaction 1

```{r no-interaction against interaction climate model 1}
#| echo: false
anova(no_interaction_model1, interaction_model1, test = "Chisq")
```

###### Species vs Species and Climate 2

```{r Just species against climate 2}
#| echo: false
anova(no_climate_model, no_interaction_model2, test = "Chisq")
```

###### Species-Climate Interaction 2

```{r No-interaction against interaction climate model 2}
#| echo: false
...
```{r Just climate against climate and species model}
#| echo: false
...
#| echo: false
#| fig-cap: "Figure 15: Maps comparing the Present GLM Occurrences and Distributions of Species 1 with those based on Climate Projections for 2060-2081"

# Plots results side by side
par(mfrow = c(2, 2))

# Present distribution
plot(species1_pred, main = "Present GLM Probability of Occurrence")
plot(world_map, add = TRUE, border = "darkgrey")

# Future distribution
plot(species1_future_pred, main = "Future GLM Probability of Occurrence")
plot(world_map, add = TRUE, border = "darkgrey")
...
[Range Change Metrics:]{.underline}
...
#| echo: false
#| fig-cap: "Figure 16: Maps comparing the present GLM Occurrences and Distributions of Species 2 with those based on Climate Projections for 2060-2081"

par(mfrow = c(2,2))

# Present distribution
plot(species2_pred, main = "Present GLM Occurrence")
plot(world_map, add = TRUE, border = "darkgrey")

# Future distribution
plot(species2_future_pred, main = "Future GLM Occurrence")
plot(world_map, add = TRUE, border = "darkgrey")
...
#| echo: false
#| fig-cap: "Figure 17: Maps comparing the present GLM Occurrence overlap and Distributions of Overlap  with those based on Climate Projections for 2060-2081"

par(mfrow = c(2,2))

# Present overlap
plot(combined, main = "Present overlap of GLM Occurrence")
plot(world_map, add = TRUE, border = "darkgrey")

# Future overlap
plot(combined_future, main = "Future overlap of GLM Occurrence")
plot(world_map, add = TRUE, border = "darkgrey")
...
#| echo: false
print(paste("Future Degree of Overlap =", future_overlap_metric))
```
...
Appel, C. L., Moriarty, K. M., Matthews, S. M., Green, D. S., King, S. A., Yaeger, J. S., . . . Bean, W. T. (2021). NORTH AMERICAN PORCUPINE DISTRIBUTION IN THE PACIFIC NORTHWEST AND EVALUATION OF A NON-INVASIVE MONITORING TECHNIQUE. *Northwestern Naturalist, 102*(1), 9-29. 
<https://doi.org/10.1898/1051-1733-102.1.9>
...
NABCI. (2016). *State of North America's Birds 2016.* Ottowa, Ontario: Environment and Climate Change Canada. 
